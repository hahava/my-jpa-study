    ...
    - 값 타입
        ...
        - 3가지 분류가 가능하다
            ...
            - 임베디드 타입
                ...
                - 기본생성자 필수?
            - 컬렉션 타입
                - 관계형 데이터베이스에서 컬렉션을 어떻게 표현할 것 인가??
                    - 같은 테이블안에 넣을 수 있는 방법은 없다.
                    - 최신 mysql 에선 json을 지원하긴 한다.
                - 1:N 개념이으로 생각하면 된다.
                    - @ElementCollection로 정의하며 @CollectionTable로 테이블 설정할 수 있다.
...
- 모든 DB 데이터를 객체로 변환해서 검색하는 것은 불가능
    - 따라서 JPA는 다양한 쿼리 방법을 지원한다.
        - JPQL(Java persistence query language)
            - JPQL을 사용한다 하여도 테이블을 대상으로 하는것이 아닌 객체를 대상으로 한다.
                - SQL을 추상화해서 특정 데이터베이스 SQL에 의존하지 않는다.
            - 문법
                - 엔터티와 속성은 대소문자를 구분하지만 문법은 구분하지 않는다.
                - 테이블이 아니라 엔터티 이름으로 사용한다.
                - 별칭은 필수(m) (as 생략가능)
                - TypeQuery => 반환 타입이 명확한 경우
                - Query => 타입이 불분명확한 경우 
        - JPA Criteria
            - query문은 결국 String 이다. 동적 쿼리에서 이러한 부분은 문제가 발생할 수 있을 가능성이 있다.
            - 많이 사용하나? 표준 스펙은 맞지만 `실무에서는 잘 사용할 수 있을지 의문`이다.
        - QueryDSL
